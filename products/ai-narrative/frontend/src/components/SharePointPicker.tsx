...
const SharePointPicker = ({ token, onSelect, acceptExtensions, disabled }: Props) => {
  ...
  useEffect(() => { void checkStatus(); }, [token]);
  ...
  useEffect(() => {
    if (selectedLibrary) void loadFiles(selectedLibrary, "/");
  }, [selectedLibrary]);
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      if (!status.configured) { setState("unconfigured"); return; }
  ...
  const isAccepted = (file: SharePointFile) =>
    !acceptExtensions?.length || acceptExtensions.includes(fileExt(file.name));

  if (state === "checking") return (
    <div className="sp-picker sp-picker-status">
      <Loader2 size={16} className="spin" /><span>Connecting to SharePoint…</span>
    </div>
  );

  if (state === "unconfigured") return (
    <div className="sp-picker sp-picker-status sp-picker-warn">
      <span>SharePoint is not configured. Add <code>SHAREPOINT_*</code> credentials to your <code>.env</code> file.</span>
    </div>
  );
    ...
    <div className={`sp-picker${disabled ? " sp-picker-disabled" : ""}`}>
      <div className="sp-toolbar">
        <select
          ...
          onChange={(e) => { setSelectedLibrary(e.target.value); setBreadcrumbs([]); }}
          disabled={disabled}
        >
          {libraries.map((lib) => <option key={lib.id} value={lib.id}>{lib.name}</option>)}
        </select>
        <button type="button" className="sp-refresh-btn"
          onClick={() => void loadFiles(selectedLibrary, folderPath)}
          disabled={disabled || loadingFiles} title="Refresh">
          <RefreshCw size={13} className={loadingFiles ? "spin" : ""} />
        </button>
      </div>

      <div className="sp-breadcrumb">
        <button type="button" className="sp-breadcrumb-item"
          onClick={() => { setBreadcrumbs([]); void loadFiles(selectedLibrary, "/"); }}
          disabled={disabled}>Root</button>
        {breadcrumbs.map((crumb, idx) => (
          <span key={crumb.path} className="sp-breadcrumb-sep-row">
            <ChevronRight size={12} className="sp-breadcrumb-sep" />
            <button type="button" className="sp-breadcrumb-item"
              onClick={() => handleBreadcrumb(crumb.path, idx + 1)} disabled={disabled}>
              {crumb.name}
            </button>
          </span>
        ))}
      ...
      <div className="sp-file-list">
        {loadingFiles ? (
          <div className="sp-file-loading"><Loader2 size={14} className="spin" /> Loading…</div>
        ) : items.length === 0 ? (
          ...
          return (
            <button key={item.id} type="button"
              ...
              disabled={disabled || (!item.is_folder && !accepted)} title={item.name}>
