...
def _build_ollama_catalog() -> dict[str, object]:
    ...
    configured_default = env("OLLAMA_CHAT_MODEL", OLLAMA_DEFAULT_MODEL) or OLLAMA_DEFAULT_MODEL
    if not enabled:
        return _empty_catalog("ollama", "Ollama", configured_default)

    source_message: str | None = None
    try:
        model_ids = _fetch_ollama_model_ids()
        source = "live"
        # In live mode: only show models that are actually installed.
        # Pick the best available default from the live list, ignoring any
        # configured default that has not been pulled.
        models = _curate_models("ollama", model_ids, configured_default, insert_default=False)
        if not models:
            # Nothing survived filtering (e.g. only embedding models installed)
            models = [_model_dict("ollama", configured_default, configured_default, True)]
            source = "curated_fallback"
        # Pick the effective default: prefer configured if available, else first fallback that is installed
        live_ids = {str(m["id"]) for m in models}
        preferred = [configured_default] + list(OLLAMA_FALLBACK_MODELS)
        effective_default = next((m for m in preferred if m in live_ids), str(models[0]["id"]))
        for m in models:
            m["is_default"] = (str(m["id"]) == effective_default)
    except Exception as exc:
        model_ids = _fallback_model_ids("ollama", configured_default)
        source = "curated_fallback"
        source_message = str(exc)
        models = _curate_models("ollama", model_ids, configured_default, insert_default=True)
        if not models:
            models = [_model_dict("ollama", configured_default, configured_default, True)]
    ...
    return {
        ...
        "display_name": "Ollama",
        "enabled": True,
        "default_model": _resolve_default_model(effective_default, models),
        "models": models,
        "source": source,
        "source_message": source_message,
    }
...
def _ollama_provider_enabled() -> bool:
    if env("OLLAMA_ENABLED") is not None:
        return env_bool("OLLAMA_ENABLED", False)
    return False
...
def _curate_models(provider: ProviderName, model_ids: list[str], default_model: str, insert_default: bool = True) -> list[dict[str, object]]:
    ...
    if insert_default and default_model and default_model not in filtered_ids:
